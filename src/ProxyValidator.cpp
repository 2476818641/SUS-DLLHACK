    ...
    : QObject(parent)
{
    ...
    // 宿主进程提前退出时立即读取日志，不再等待计时器
    connect(&m_hostProcess, qOverload<int, QProcess::ExitStatus>(&QProcess::finished),
            this, [this](int /*exitCode*/, QProcess::ExitStatus) {
        if (m_hostProcess.state() != QProcess::NotRunning) {
            return; // 计时器已经处理，忽略
        }
        // 进程已退出，仍等待计时器 finalise（计时器会检查进程状态并跳过 terminate）
    });
}
...
void ProxyValidator::prepareSandbox()
{
    ...
    // 递归复制宿主目录（含子目录，如 platforms/、imageformats/ 等插件目录）
    QDirIterator files(hostDir.absolutePath(),
                       QDir::Files | QDir::NoDotAndDotDot,
                       QDirIterator::Subdirectories);
    while (files.hasNext()) {
        const QString source = files.next();
        const QString rel    = hostDir.relativeFilePath(source);
        const QString target = QDir(m_sandboxDir).filePath(rel);
...
void ProxyValidator::startHost()
{
    ...
    QTimer::singleShot(6000, this, [this]() {
        ...
        const bool proxyLoaded = readProxyLoaded();
        const QStringList called = readCalledExports();
        emit validationFinished(proxyLoaded, called, m_logPath, m_proxy.sourcePath);
    });
}
...
bool ProxyValidator::readProxyLoaded() const
{
    QFile file(m_logPath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        return false;
    ...
    const QString text = QString::fromUtf8(file.readAll());
    return text.contains(QStringLiteral("=== proxy loaded ==="));
}
